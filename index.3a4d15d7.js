 ...
 * limitations under the License.
 */
function(e,t){const n=ke(e,"auth");if(n.isInitialized()){const e=n.getImmediate();if(R(n.getOptions(),null!=t?t:{}))return e;Ve(e,"already-initialized")}return n.initialize({options:t})}(e,{popupRedirectResolver:zr,persistence:[Qn,jn,Wn]}),r=x("authTokenSyncURL");if(r&&"boolean"==typeof isSecureContext&&isSecureContext){const e=new URL(r,location.origin);if(location.origin===e.origin){const t=(i=e.toString(),async e=>{const t=e&&await e.getIdTokenResult(),n=t&&((new Date).getTime()-Date.parse(t.issuedAtTime))/1e3;if(n&&n>jr)return;const r=null==t?void 0:t.token;Ur!==r&&(Ur=r,await fetch(i,{method:r?"POST":"DELETE",headers:r?{Authorization:`Bearer ${r}`}:{}}))});!function(e,t,n){U(e).beforeAuthStateChanged(t,n)}(n,t,(()=>t(n.currentUser))),function(e,t,n,r){U(e).onIdTokenChanged(t,n,r)}(n,(e=>t(e)))}}var i;const o=b("auth");return o&&sn(n,`http://${o}`),n}(Se({apiKey:"AIzaSyBFTleaabEETsFQRavrze5fpw5vtJK-FIY",authDomain:"appgame-7b22b.firebaseapp.com",projectId:"appgame-7b22b",storageBucket:"appgame-7b22b.appspot.com",messagingSenderId:"55180818662",appId:"1:55180818662:web:bf20405c5c03c79a0e2327",measurementId:"G-GSFXDHQHQ4"}))),qr=()=>{Vr.value.trim()&&Fr.value.trim()?Hr.disabled=!1:Hr.disabled=!0};Fr.addEventListener("input",qr),Vr.addEventListener("input",qr),qr(),Hr.addEventListener("click",(e=>{e.preventDefault(),Hr.disabled=!0,Dn($r,Fr.value,Vr.value).then((e=>{const t=e.user;console.log("User registered:",t)})).catch((e=>{const t=e.message;console.error("Registration error:",t)}))}));const Xr=document.querySelector(".hamburger-checbox"),Kr=document.querySelector(".btns-and-theme-hamb");Xr.addEventListener("change",(()=>{Kr.classList.toggle("show-menu")}));const Gr=document.querySelector(".login-container"),Jr=document.querySelector(".buttons-register li:nth-of-type(1) button"),Yr=document.querySelector(".close-btn");function Qr(){Jr.style.color="",Gr.style.display="none"}Jr.addEventListener("click",(e=>{e.preventDefault(),Jr.style.color="rgb(0, 255, 213)",Gr.style.display="block"})),Yr.addEventListener("click",(e=>{e.preventDefault(),Qr()})),document.addEventListener("keydown",(e=>{"Escape"===e.key&&Qr()}));const Zr=document.querySelector(".next-page-button"),ei=document.querySelector(".prev-page-button");const ti=document.querySelector(".main-gallery"),ni=document.querySelector(".loader");async function ri(e){const t=new URL("games","https://api.rawg.io/api/");t.searchParams.append("ordering","-rating"),t.searchParams.append("key","8b50ecc01425485f9653061e73d88e87"),t.searchParams.append("page",e);try{const e=await fetch(t);return await e.json()}catch(e){console.error("Error fetching popular games:",e)}}async function ii(){const e=new URL("https://api.rawg.io/api/genres");e.searchParams.append("key","8b50ecc01425485f9653061e73d88e87");try{const t=await fetch(e);return(await t.json()).results}catch(e){console.error("Error fetching genres:",e)}}async function oi(e){ti.innerHTML="";for(const t of e){const{id:e,name:n,background_image:r,genres:i,tags:o,rating:a,added:s,released:c,parent_platforms:l,short_screenshots:u}=t;if(o.some((e=>"nsfw"===e.name.toLowerCase())))continue;const f=new Image;f.src=r,f.onload=async function(){if(this.naturalWidth>=this.naturalHeight){const u=i.map((e=>e.name)),f=(o.map((e=>e.name)),l.map((e=>e.platform.name)).slice(0,2)),d=`\n          <li class="card-container" data-id="${e}">\n            <div class="first-content-card" data-id="${e}" data-title="${n}" data-original_title="${n}">\n              <div class="img-content">\n                  <img class="cards-home-game-image" width="310" height="170" src="${r}" alt="Poster ${n}" />\n                <div>\n                <div class="stats-cards">\n                <p class="cards-home-game-rating">${a}</p>\n                <p class="cards-home-game-added">+ ${s}</p>\n                </div>\n                <span class="cards-home-game-title">${n.toUpperCase()}</span>\n                </div>\n              </div>\n            </div>\n            <div class="second-content-card">\n            ${u.length>0?`<div class="cards-home-game-genres"><span>Genres:</span> ${u.join(", ")}</div>`:""}\n            ${c?`<div class="cards-home-game-genres"><span>Release date:</span> ${c}</div>`:""}\n            ${f.length>0?`<div class="cards-home-game-genres"><span>Platforms:</span> ${f.join(", ")}</div>`:""}\n              <button class="view-details-btn" data-id="${e}">View Details</button>\n            </div>\n          </li>\n        `;ti.insertAdjacentHTML("beforeend",d),ni.style.display="none",Zr.style.display="block",ei.style.display="block",document.querySelector(`[data-id="${e}"] .view-details-btn`).addEventListener("click",(async function(){ai(t)}))}}}}async function ai(e){const t=await async function(e){const t=new URL(`games/${e}`,"https://api.rawg.io/api/");t.searchParams.append("key","8b50ecc01425485f9653061e73d88e87");try{const e=await fetch(t);return(await e.json()).description}catch(t){return console.error(`Error fetching game description with id ${e}:`,t),""}}(e.id),n=e.short_screenshots.slice(0,6);document.querySelector("body").style.overflow="hidden";const r=n.map(((e,t)=>`<img src="${e.image}" alt="Screenshot ${t+1}" width="300" />`)).join(""),i=`\n    <div class="modal-content" style="background-image: url(${e.background_image}); background-repeat: no-repeat; background-size: cover;">\n      <div class="gradient-overlay"></div>\n      <div class="follow-us"><span>Follow us!</span></div>\n      <span class="close">&times;</span>\n\n      <div class="container-modal-div">\n        <div class="screenshots-modal">\n\n          ${r}\n\n        </div>\n        <div class="stats-modal">\n          <h2>${e.name}</h2>\n          <ul class="buttons-modal">\n            <li>\n              <button>\n                Add to <span>Wishlist</span>\n                <span class="stat-modal-btn">${e.added}</span>\n              </button>\n            </li>\n            <li><button>Add to my <span>games</span></button></li>\n          </ul>\n          <div class="stats-modal-styles">\n            <p>Rating: <span>${e.rating}</span></p>\n            <p>Added: <span>${e.added}</span></p>\n          </div>\n          ${e.genres.length>0?`\n            <p>Genres: <span>${e.genres.map((e=>e.name)).join(", ")}</span></p>\n          `:""}\n          ${e.tags.length>0?`\n            <p>Tags: <span>${e.tags.map((e=>e.name)).join(", ")}</span></p>\n          `:""}\n          <div class="description-content">\n          <p>Description: ${t}</p>\n          </div>\n          <p class="view-more-screenshots"><span class="effect">see screenshots</span> </p>\n\n        </div>\n      </div>\n    </div>\n  `;document.querySelector(".modal").innerHTML=i,document.querySelector(".modal").style.display="block";document.querySelector(".modal-content .close").addEventListener("click",si)}async function si(){document.querySelector(".modal").style.display="none",document.querySelector("body").style.overflow="auto"}Zr.style.display="none",ei.style.display="none",document.addEventListener("keydown",(e=>{"Escape"===event.key&&si()})),ri(1).then((e=>e.results)).then(oi).catch((e=>console.error("Error fetching popular games:",e)));let ci=1;const li=document.querySelector(".prev-page-button"),ui=document.querySelector(".next-page-button");li.addEventListener("click",(async()=>{if(ci>1)try{oi((await ri(ci-1)).results),ci--,window.scrollTo({top:0,behavior:"smooth"})}catch(e){console.error("Error fetching and rendering games:",e)}})),ui.addEventListener("click",(async()=>{try{oi((await ri(ci+1)).results),ci++,window.scrollTo({top:0,behavior:"smooth"})}catch(e){console.error("Error fetching and rendering games:",e)}}));const fi=document.querySelector("#search-form"),di=document.querySelector("#search-input"),pi=document.querySelector(".hero-heading");fi.addEventListener("submit",(async e=>{e.preventDefault(),ti.innerHTML="",li.style.display="none",ui.style.display="none",ni.style.display="block",pi.innerHTML="";const n=di.value.trim();if(n){try{const e=await async function(e,t){const n=new URLSearchParams({search:e,page_size:40,key:"8b50ecc01425485f9653061e73d88e87",image_type:"photo",safesearch:!0,page:t}),r=await fetch(`https://api.rawg.io/api/games?${n.toString()}`);if(!r.ok)throw new Error(r.statusText);return r.json()}(n,1);if(pi.innerHTML=`search for : ${n}`,ni.style.display="none",!e.results.length)return t(c).Notify.info("No games found"),li.style.display="none",ui.style.display="none",void(pi.innerHTML="No games found");li.style.display="block",ui.style.display="block",oi(e.results)}catch(e){console.error(e),t(c).Notify.failure("Failed to fetch games")}fi.reset()}})),async function(){const e=await ii(),t=document.querySelector("#genre-list");e.forEach((e=>{const n=document.createElement("li"),r=document.createElement("span");n.dataset.value=e.id,r.textContent=e.name,t.appendChild(n),n.appendChild(r),n.addEventListener("click",(async()=>{ti.innerHTML="",li.style.display="none",ui.style.display="none",ni.style.display="block";const t=n.dataset.value;pi.innerHTML=`${e.name}`;const r=await async function(e){const t=new URL("https://api.rawg.io/api/games");t.searchParams.append("key","8b50ecc01425485f9653061e73d88e87"),t.searchParams.append("genres",e);try{const e=await fetch(t);return(await e.json()).results}catch(e){console.error("Error fetching games for genre:",e)}}(t);oi(r),li.style.display="block",ui.style.display="block",ni.style.display="none"}))}))}();const hi=document.querySelector(".singup-container"),mi=document.querySelector(".buttons-register li:nth-of-type(2) button"),gi=document.querySelector(".singup-container .close-btn"),vi=()=>{mi.style.color="",hi.style.display="none"};mi.addEventListener("click",(e=>{e.preventDefault(),mi.style.color="rgb(0, 255, 213)",hi.style.display="block"})),gi.addEventListener("click",(e=>{e.preventDefault(),vi()})),document.addEventListener("keydown",(e=>{"Escape"===e.key&&vi()}));const yi=document.querySelector('.switch input[type="checkbox"]'),bi=document.querySelector("body");"light"===localStorage.getItem("theme")&&(bi.classList.add("light-theme"),yi.checked=!0),yi.addEventListener("change",(()=>{bi.classList.toggle("light-theme"),yi.checked?localStorage.setItem("theme","light"):localStorage.removeItem("theme")}));const wi=document.querySelector("#userModal"),xi=document.querySelector(".user-icon");document.querySelector(".user-profile");xi.addEventListener("click",(e=>{wi.style.display="block"}));wi.querySelector(".user-modal-close").addEventListener("click",(e=>{wi.style.display="none"}));const ki=document.getElementById("profilePicInput"),_i=document.getElementById("userIcon"),Ii=document.querySelector(".background-image"),Ni=document.querySelector(".bg-image");let Si=!1;_i.addEventListener("click",(()=>{Si||(ki.click(),Si=!0)})),Ii.addEventListener("click",(()=>{Si||(Ii.click(),Si=!0)})),ki.addEventListener("change",(e=>{const t=e.target.files[0];if(t){const e=URL.createObjectURL(t);_i.src=e,_i.style.borderRadius="50%"}Si=!1})),Ii.addEventListener("change",(e=>{const t=e.target.files[0];if(t){const e=URL.createObjectURL(t);Ni.style.backgroundImage=`linear-gradient(to bottom, rgba(0,0,0,0.5), rgba(0,0,0,0.7)), url(${e})`}Si=!1}));
//# sourceMappingURL=index.3a4d15d7.js.map
